		...
		if waitForLabeledPodsGone != "" {
			...
			if pods != nil && len(pods.Items) != 0 {
				for _, pod := range pods.Items {
					if pod.DeletionTimestamp != nil && time.Since(pod.DeletionTimestamp.Time).Minutes() < float64(5) {
						log.Info(fmt.Sprintf("pods with label %s still exist in namespace %s, retrying...", labelSelectors, namespace))
						break
					}
				}
			}
